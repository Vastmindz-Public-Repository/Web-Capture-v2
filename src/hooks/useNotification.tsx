...
import { Notification, NOTIFICATION_TIMEOUT, prioritizeNotifications } from 'helpers/notification'
...
const useNotification = () => {
  ...
  const addNotification = (message: string, discrete = false) =>
    setNotifications((notifications) => {
      const notification = notifications.find(item => item.message === message)

      if (notification) {
        clearTimer(notification)
        notification.timer = discrete ? null : createTimer(notification.message)
      ...
      return prioritizeNotifications([
        ...
        {
          message,
          timer: discrete ? null : createTimer(message),
        },
      ])
    })
